...
# Load two background images and resize them
bg_image1 = Image.open("C:/Users/Deepu/Desktop/python/portfolio/technology-6701504_640.webp")  # Replace with your first background image path
bg_image1 = bg_image1.resize((1600, 800), Image.LANCZOS)  # Resize image to double the width
bg_image1_tk = ImageTk.PhotoImage(bg_image1)

bg_image2 = Image.open("C:/Users/Deepu/Desktop/python/portfolio/digitization-1599552_640.webp")  # Replace with your second background image path
bg_image2 = bg_image2.resize((1600, 800), Image.LANCZOS)  # Resize image to double the width
bg_image2_tk = ImageTk.PhotoImage(bg_image2)

# Create images on the canvas
bg_id1 = canvas.create_image(0, 0, anchor="nw", image=bg_image1_tk)
bg_id2 = canvas.create_image(1600, 0, anchor="nw", image=bg_image2_tk)

def move_background():
    # Move both background images
    canvas.move(bg_id1, -10, 0)  # Move the first background image left by 10 pixels
    canvas.move(bg_id2, -10, 0)  # Move the second background image left by 10 pixels

    # Check if the first background image is out of view and reset position
    if canvas.coords(bg_id1)[0] <= -1600:
        canvas.coords(bg_id1, 1600, 0)  # Reset position to the right of the second image

    # Check if the second background image is out of view and reset position
    if canvas.coords(bg_id2)[0] <= -1600:
        canvas.coords(bg_id2, 1600, 0)  # Reset position to the right of the first image

    # Repeat after 20 ms for faster animation
    canvas.after(20, move_background)
...
about.pack(pady=10)
about_text = tk.Label(content_frame, text="I am a passionate developer with experience in Python and AI", bg="#f0f8ff", fg="#333333")
...
contact_info = tk.Label(content_frame, text="Email: nagadeepak1712@example.com", bg="#f0f8ff", fg="#333333")
